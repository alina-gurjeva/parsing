...
class Post(Base, UrlMixin):
    __tablename__ = "post"
    id = Column(Integer, primary_key=True, autoincrement=True)  # not from data, only autoincrement
    title = Column(String(250), nullable=False, unique=False)
    author_id = Column(Integer, ForeignKey("author.id"), nullable=True)
    id_comments = Column(Integer, ForeignKey("comment.id"), nullable=True)
    img_url = Column(String, unique=False, nullable=True)  # it is possible no image in a post
    date_published = Column(DateTime, unique=False, nullable=False)
    # relationships:
    author = relationship("Author", foreign_keys=[author_id])
    tags = relationship("Tag", secondary=tag_post)
    comments = relationship("Comment", foreign_keys=[id_comments])
...
class Comment(Base):
    ...
    author = relationship("Author", backref="comments", foreign_keys=[author_id])
    time_now = Column(DateTime)
    post_id = Column(Integer, ForeignKey("post.id"))
    post = relationship(Post, foreign_keys=[post_id])
    comment_to_comment = relationship("Comment", foreign_keys=[parent_id])
